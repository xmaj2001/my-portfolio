...
import { LANG_COLORS, type Repo } from '@/features/projects/type';
...
export function FeatureCard({ repo, className, ...props }: FeatureCardProps) {
	const p = genRandomPattern();

	return (
		<Link href={`/projects/${repo.name}`} className="block no-underline">
			<article className={cn('relative overflow-hidden border border-[#21262d] bg-[#0d1117] p-6 transition-colors hover:border-[#58a6ff]', className)} {...props}>
				...
				<div
					...
					style={repo.coverUrl ? { background: `url(${repo.coverUrl}) center/cover` } : undefined}
				>
					{!repo.coverUrl && (
				...
				<h3 className="relative z-20 mt-5 text-sm font-semibold md:text-base">{repo.name}</h3>
				...
				{repo.topics.length > 0 && (
					<div className="relative z-20 mt-3 flex flex-wrap gap-1.5">
						...
						{repo.topics.length > 3 && (
							<span className="px-1 text-[11px] text-[#8b949e]">+{repo.topics.length - 3}</span>
						)}
					</div>
				)}
...
function IconForLanguage({ language }: { language: string }) {
	return <span className="text-5xl opacity-30">{getIconForLanguage(language)}</span>;
}
			...
			{squares && (
				<svg x={x} y={y} className="overflow-visible" aria-hidden="true">
					{squares.map(([sx, sy], index) => (
						<rect strokeWidth="0" key={`${sx}-${sy}-${index.toString()}`} width={width + 1} height={height + 1} x={sx * width} y={sy * height} />
					))}
				</svg>
			)}
		</svg>
